import 'package:el_shaddai_edu_portal/school_fees_screen/domain/bloc/school_fees_bloc.dart';
...
class _SchoolFeesScreenState extends State<SchoolFeesScreen> {
  ...
  void createDataSource() {
    ...
    headers = [
      ...
      for (var month in schoolMonths) ExpandableColumn<String>(
          columnTitle: month, columnFlex:11),
    ];

    // Populate sample rows (you should replace this with your data)
    rows = [
      ExpandableRow(cells: [
        ExpandableCell<String>(columnTitle: "ID", value: "1"),
        ExpandableCell<String>(columnTitle: "Name", value: "John Doe"),
        for (var _ in schoolMonths) ExpandableCell<String>(
            columnTitle: _, value: "0"),
      ]),
                  ...
                  child: ExpandableDataTable(
                    ...
                    rows: state.studentsList.map<ExpandableRow>((e) {
                      List<ExpandableCell<dynamic>> cells = [
                        ...
                        ExpandableCell<String>(columnTitle: "Nom", value: e.name),
                        for (var _ in schoolMonths) ExpandableCell<String>(
                            columnTitle: _, value: e.schoolFees?[_]?.toString() ?? '0'),
                      ];
                    ...
                    onRowChanged: (newRow) {
                      ...
                      for (var i = 2; i < newRow.cells.length; i++) {
                        schoolFees[headers[i].columnTitle] = int.tryParse(newRow.cells[i].value) ?? 0;
                      }
                      context.read<SchoolFeesBloc>().add((OnSchoolFeesUpdated(studentId, schoolFees)));
                      // Handle row changes
                    },
